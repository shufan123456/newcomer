 ...
 import com.tuhu.sf.newcomer.task.service.common.RedisKey;
 ...
 public class MultiThreadingCreatEnroll {
     ...
     public void createEnroll() {
         ...
         try {
             //报名队列取一个出栈
             AccountQueueStatus accountQueueStatus = (AccountQueueStatus) redisTemplate
                     .boundListOps(RedisKey.ACCOUNT_QUEUE_LIST.getCode()).rightPop();
             ...
             Object obj = redisTemplate.boundListOps(RedisKey.COURSE_COUNT.getCode() + "_" +
                     accountQueueStatus.getCourseId()).rightPop();
             if (null == obj) {
                 //设置为审核失败状态
                 setQueueStatus(accountQueueStatus);
             ...
             //把生成的id写回redis的用户报名状态里面去-->方便后续清理掉这些没有id的key
             accountQueueStatus.setEnrollId(enroll.getId());
             redisTemplate.boundHashOps(RedisKey.ACCOUNT_QUEUE_STATUS.getCode() + "&" +
                     accountQueueStatus.getAccountId()).put(accountQueueStatus.getCourseId(),accountQueueStatus);
             //需要再次判断一下，席位的数量。
             Long size = redisTemplate.boundListOps(RedisKey.COURSE_COUNT.getCode() + "_" +
                     accountQueueStatus.getCourseId()).size();
             if (size <= 0) {
                 //需要清理redis中的课程
                 redisTemplate.boundHashOps(RedisKey.COURSE_KEY.getCode()).delete(accountQueueStatus.getCourseId());
             }
         } catch (Exception e) {
             e.printStackTrace();
         } finally {
     ...
     public void clearAccountQueue(Long accountId, Long courseId) {
         ...
         //排队信息状态清理
         redisTemplate.boundHashOps(RedisKey.ACCOUNT_QUEUE_STATUS.getCode() + "&" + accountId).delete(courseId);
     }
